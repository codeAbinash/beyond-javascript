...
export default function load(src = 'index/Javascript Home', scrollAmount = 0) {
  ...
  fetch(fetchLink)
    ...
    .then(([text, err]) => {
      if (err) {
        main.innerHTML = `<h1> <span class="gT">404</span> : Page Not found</h1>
                    <div class="btnDiv">
...
async function displayMeme() {
  ...
  try {
    ...
    memeText =
      'Meme cannot be loaded. Maybe there is some error occurred. Check your internet connection or maybe the location of the meme json file is changed';
    memeSrc = false;
    noFun = true;
  }
  ...
  if (memeSrc) {
    ...
    for (let i = 0; i < emojiNumbers; i++) emojiContainer.appendChild(createEmojiDOM());
    if (noFun) emojiContainer.classList.add('hidden');
  }
}
...
function loadPageLink() {
  ...
  elements.forEach((elem) => {
    ...
    elem.addEventListener('click', (e) => {
      if (elem.getAttribute('target') == '_blank') return;
      e.preventDefault();
      load(link);
    });
  });
...
export function activeSideBarElements(src = 'index/Javascript Home') {
  ...
  let timeOut = 0;
  setTimeout(() => {
    if (activeElement) activeElement.scrollIntoView({ block: 'center', behavior: 'smooth' });
  }, timeOut);

  if (lastActiveElement) lastActiveElement.classList.remove('activeOption');
  if (activeElement) activeElement.classList.add('activeOption');
}
